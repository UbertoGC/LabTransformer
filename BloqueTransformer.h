...
private:
    ...
    void AprenderFeedForward(Matriz2D<N>&, N&, Matriz2D<N>&, int&);
public:
...
template <typename N>
void BloqueTransformer<N>::AprenderFeedForward(Matriz2D<N>& grad_sig, N& t_a, Matriz2D<N>& grad_feedforward, int& t_adam){
    Matriz2D<N> grad_normal_2 = DerNormalizarFilas(salida_feedforward + salida_norm_1, grad_sig);
    feedforward->Aprender(grad_normal_2, t_a, grad_feedforward, t_adam);
}
template <typename N>
...
template <typename N>
void BloqueTransformer<N>::SelfAprender(Matriz2D<N>& grad_sig, N& t_a, Matriz2D<N>& grad_salida, int& t_adam){
    Matriz2D<N> grad_feedforward;
    this->AprenderFeedForward(grad_sig, t_a, grad_feedforward, t_adam);
    Matriz2D<N> grad_normal_1 = DerNormalizarFilas(salida_atencion + (*entrada_bloque), grad_feedforward);
    atencion->SelfAprender(grad_normal_1, t_a, grad_salida, t_adam);
}
template <typename N>
void BloqueTransformer<N>::CrossAprender(Matriz2D<N>& grad_sig, N& t_a, Matriz2D<N>& grad_decoder, Matriz2D<N>& grad_encoder, int& t_adam){
    Matriz2D<N> grad_feedforward;
    this->AprenderFeedForward(grad_sig, t_a, grad_feedforward, t_adam);
    Matriz2D<N> grad_normal_1 = DerNormalizarFilas(salida_atencion + (*entrada_decoder), grad_feedforward);
    atencion->CrossAprender(grad_normal_1, t_a, grad_decoder, grad_encoder, t_adam);
}
template <typename N>
